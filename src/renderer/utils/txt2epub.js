...
export default class Txt2Epub {
    // 读取 TXT 文件内容，支持指定编码
    static async readTxtFile(filePath, encoding = 'utf8') {
        try {
     ...
     */
    static async convertTxtToEpub(txtFilePath, epubFilePath) {
        try {
            const encoding = await this.detectEncoding(txtFilePath); // 自动检测编码
            console.log(`检测到文件编码为: ${encoding}`);
            const content = await this.readTxtFile(txtFilePath, encoding);
            ...
            async function processContent(content) {
                ...
                if (chapters.length > 0) {
                    ...
                    lastChapterContent = this.formatText(lastChapterContent);
